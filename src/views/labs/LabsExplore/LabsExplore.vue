...
<script lang="ts">
  ...
  export default class LabsExplore extends Mixins(FetchMixin) {
    ...
    async fetch() {
      ...
      // this.labs = res.labs;
      if (this.$vxm.pagination.navigation === navigationModes.NAVIGATION_SCROLL && this.$route.query.skip) {
        res.labs.forEach((newLab) => {
          if (!this.labs.some((lab) => lab.nid === newLab.nid)) {
