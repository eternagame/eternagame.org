...
<script lang="ts">
  ...
  export default class ScriptView extends Mixins(FetchMixin, ScriptExecutionMixin) {
    ...
    async evaluate() {
      ...
      this.srcdoc = await this.makeScriptExecutionFrame(`
        ScriptInterface.evaluate_script_with_nid(
          ...
          ${JSON.stringify(Object.fromEntries(this.inputs.map(input => [input.name, input.value.replaceAll('\\', '\\\\').replaceAll('\n', '\\n')])))},
          function(result) {
